...
public class LetterDao {
	...
	//받은
	static final String RECEIVE_LETTERS = "select letterId, title, content, senderId, senderName, left(cdate,19) cdate from letter where receiverId=? order by letterId desc limit ?,?";
	//보낸
	static final String SEND_LETTERS = "select letterId, title, content, receiverId, receiverName, left(cdate,19) cdate from letter where senderId=? order by letterId desc limit ?,?";
	...
	//읽기
	static final String GET_LETTERS = "select letterId, title, content, senderId, senderName, receiverId, receiverName, left(cdate,19) cdate from letter where letterId=? and (senderId=? or receiverId=?)";
	//추가(쓰기)
